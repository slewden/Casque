...
app.directive('colorChooser', ['$filter', 'ModalService', function ($filter, ModalService) {
  return {
    ...
    template:
              '<div style="display:inline-block;width:100px;height:20px;border:1px solid #827878;padding: 1px;cursor:pointer;background-color:white;" data-ng-click="choixColor()">'
     ...
     + '  <span style="padding-left: 4px;background-color:white;"><i class="fa fa-caret-down"></i></span>'
     + '</div>',
    link: function (scope, element, attr, ctrl) {
      ...
      var getOffset = function (el) {
        ...
        return { top: y - px, left: x -px, width: w, height: h };
      }
      //-- choix date valide
...
app.controller('colorPiker', ['$scope', '$timeout', 'webStorage', 'color', 'key', 'position', 'close', function ($scope, $timeout, webStorage, color, key, position, close) {
  ...
  $scope.position = { left: Math.max(0, Number(position.left) + Number(position.width) - 223), top: Number(position.top) + Number(position.height) };
}]);
//-- Controlleurs ------------------------------------------------------------------------------------
