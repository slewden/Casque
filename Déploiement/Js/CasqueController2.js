//-------------------------------------------------------------------------------------------------------
//-- Controlleur Pages : reception / assemblage / livraison / ... avec lecteur quoi ---------------------
app.controller('receptionController', ['$scope', '$rootScope', '$timeout', '$noHttp', 'NOCONFIG', 'webStorage', '$translate', 'parameters',
  function ($scope, $rootScope, $timeout, $noHttp, NOCONFIG, webStorage, $translate, parameters) {
    $scope.pageCode = parameters.mode;    // Type d'action attendue
    $scope.waitTagStatut = $scope.pageCode == 'livraison' ? 1 : $scope.pageCode == 'assemblage' ? 2 : 3;             // Etat des tags attendus
  ...
  function ($scope, $rootScope, $noHttp, $route, $location, webStorage, $timeout, $translate) {
    ...
    $scope.sayHello = function () {
      ...
      $scope.casqueHub.server.hello(0)
          .done(function (values) { // Recue valeur
            if ((values & 1) == 0) { // aucun encoreur disponible
              $scope.progression = 3;
